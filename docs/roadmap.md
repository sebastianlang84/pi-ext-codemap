...
| 6 | Add ranking explain guardrails before heavier search | Search pipeline/ranking Modules | `searchCodeMapWithDiagnostics` or benchmark output | A representative query exposes why top results won through path/symbol/FTS/doc/test signals | `npm test -- --test-name-pattern='ranking|diagnostic'` |
